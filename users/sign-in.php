...
if (isset($_POST['submit'])) {
  ...
  $query = "SELECT * FROM `regester` WHERE uemail = '$email' and upassword = '$password';";
  ...
  if (mysqli_num_rows($result) > 0) {
    if ($row['role'] == 0) {
      if ($row['status'] == 1) {
        ...
        echo "<script>alert('Wait till Admin Approves Your Request')</script>";
      }
    } else if ($row['role'] == 1) {
      if ($row['status'] == 1) {
        $_SESSION["username"] = $row["uname"];
    ...
    echo "<script>alert('This Email and Password Does not Exist')</script>";
  }
}
?>
