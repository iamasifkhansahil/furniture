...
if (isset($_POST['searchInput'])) {
    ...
    $query = "SELECT * FROM `regester` WHERE uname LIKE '%$searchInput%' AND role = 1;";
    $result = mysqli_query($con, $query);
} else {
    $query = "SELECT * FROM `regester` WHERE role = 1";
    $result = mysqli_query($con, $query);
}
?>
            ...
            <div class="card my-4">
                <div class="card-header p-0 position-relative mt-n4 mx-3 z-index-2">
                    <div class="bg-gradient-primary shadow-primary border-radius-lg pt-4 pb-3">
                        <h6 class="text-white text-capitalize bg-gradient-primary ps-3">Vendors table</h6>
                    </div>
                </div>
                            ...
                            <tbody>
                                ...
                                ?>
                                    <tr>
                                        ...
                                        <td class="align-middle text-center text-sm">
                                            <a href="visivendor.php?id=<?php echo $fetchdata['uid'] ?>&status=<?php echo $fetchdata['status'] ?>" class="button-30 <?php echo ($fetchdata['status'] == 1) ? 'bg-success' : 'bg-danger' ?>" role="button"><?php echo ($fetchdata['status'] == 1 ? "Active" : "Deactive") ?></a>
                                        </td>
